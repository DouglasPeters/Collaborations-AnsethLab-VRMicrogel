...
Folder = 'F:\DATA\Widefield Images-20210414T023702Z-001\Widefield Images\';
...
CellChannels = [1 2 3]; %Which channels are cell-associated? This is used for segmentation of cell are. Format: [1 2 3 4], [1 3], etc.
Ch_DAPI = 1; %Which channel corresponds to DAPI/Hoechst signal?
CoExpressionAnalysis = 1; %Do you want to perform co-expression analysis for each channel combination in the image? (1=yes, 0=no)
...
ImageSave_Erosion = 0; %Do you want to save an image showing the "erosion" outlines and cell area segmentation? (1=yes, 0=no)
ImageSave_ChannelMasks = 0; %Do you want to save images showing the cell segmentation for each channel? (1=yes, 0=no)
...
for f = START:FINISH
    ...
    if ZPlanes>1    
        ...
        Image.Ch1MIP = Image.Ch1;
        Image.Ch2MIP = Image.Ch2;
        if Channels>2, Image.Ch3MIP = Image.Ch3; else end
        if Channels>3, Image.Ch4MIP = Image.Ch4; else end
    end
    ...
    Erosion.MicrogelCh1Mask = Erosion.MicrogelCh1Image>ChannelSeg.Ch1MT(1);
    ...
    Erosion.MicrogelCh2Mask = Erosion.MicrogelCh2Image>ChannelSeg.Ch2MT(1);
    ...
    if Channels > 2
       ...
       Erosion.MicrogelCh3Mask = Erosion.MicrogelCh3Image>ChannelSeg.Ch3MT(1);
    ...
    if Channels > 3
       ...
       Erosion.MicrogelCh4Mask = Erosion.MicrogelCh4Image>ChannelSeg.Ch4MT(1);
    ...
    ErosionProps(f,1).Filename = srcFiles(f,1).name;
    for m = 1:Erosion.ErosionIterations
        %% Area and Signal Intensity Analysis per Channel and Band %%
    ...
    end %% End of Band Analysis Loop.
    
    %% Co-Expression Area Analysis (Total Microgel) %%
 %% Co-Expression Area Analysis (By Band and Total Microgel) %%
    if CoExpressionAnalysis > 0
        disp('Performing co-expression area analysis...'); 
        CoEx(f,1).Filename = srcFiles(f,1).name;
        if f > START, clearvars CoExImages; else end
    %Creates co-expression images for entire microgel%
        CoExImages.Ch1Ch2Total = and(Erosion.MicrogelCh1Mask,Erosion.MicrogelCh2Mask);
        if Channels>2
            CoExImages.Ch1Ch3Total = and(Erosion.MicrogelCh1Mask,Erosion.MicrogelCh3Mask);
            CoExImages.Ch2Ch3Total = and(Erosion.MicrogelCh2Mask,Erosion.MicrogelCh3Mask);
        else end
        if Channels>3
            CoExImages.Ch1Ch4Total = and(Erosion.MicrogelCh1Mask,Erosion.MicrogelCh4Mask);
            CoExImages.Ch2Ch4Total = and(Erosion.MicrogelCh2Mask,Erosion.MicrogelCh4Mask);
            CoExImages.Ch3Ch4Total = and(Erosion.MicrogelCh3Mask,Erosion.MicrogelCh4Mask);
        else end

    %Performs co-expression analysis for entire microgel%
        CoEx(f,1).Total.Ch1Ch2AreaSum = sum(CoExImages.Ch1Ch2Total,'all');
        CoEx(f,1).Total.Ch1Ch2PercentOfTotalCh1Area = CoEx(f,1).Total.Ch1Ch2AreaSum/sum(ErosionProps(f,1).Ch1CellAreaSum(:,1),'all')*100;
        CoEx(f,1).Total.Ch1Ch2PercentOfTotalCh2Area = CoEx(f,1).Total.Ch1Ch2AreaSum/sum(ErosionProps(f,1).Ch2CellAreaSum(:,1),'all')*100;
        CoEx(f,1).Total.Ch1Ch2PercentOfTotalCellArea = CoEx(f,1).Total.Ch1Ch2AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
        CoEx(f,1).Total.Ch1Ch2PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch1Ch2AreaSum/sum(Image.BW1,'all')*100;
        if Channels>2
            CoEx(f,1).Total.Ch1Ch3AreaSum = sum(CoExImages.Ch1Ch3Total,'all');
            CoEx(f,1).Total.Ch1Ch3PercentOfTotalCh1Area = CoEx(f,1).Total.Ch1Ch3AreaSum/sum(ErosionProps(f,1).Ch1CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch3PercentOfTotalCh3Area = CoEx(f,1).Total.Ch1Ch3AreaSum/sum(ErosionProps(f,1).Ch3CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch3PercentOfTotalCellArea = CoEx(f,1).Total.Ch1Ch3AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch3PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch1Ch3AreaSum/sum(Image.BW1,'all')*100;
            
            CoEx(f,1).Total.Ch2Ch3AreaSum = sum(CoExImages.Ch2Ch3Total,'all');
            CoEx(f,1).Total.Ch2Ch3PercentOfTotalCh2Area = CoEx(f,1).Total.Ch2Ch3AreaSum/sum(ErosionProps(f,1).Ch2CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch3PercentOfTotalCh3Area = CoEx(f,1).Total.Ch2Ch3AreaSum/sum(ErosionProps(f,1).Ch3CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch3PercentOfTotalCellArea = CoEx(f,1).Total.Ch2Ch3AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch3PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch2Ch3AreaSum/sum(Image.BW1,'all')*100;
        else end
        if Channels>3
            CoEx(f,1).Total.Ch1Ch4AreaSum = sum(CoExImages.Ch1Ch4Total,'all');
            CoEx(f,1).Total.Ch1Ch4PercentOfTotalCh1Area = CoEx(f,1).Total.Ch1Ch4AreaSum/sum(ErosionProps(f,1).Ch1CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch4PercentOfTotalCh4Area = CoEx(f,1).Total.Ch1Ch4AreaSum/sum(ErosionProps(f,1).Ch4CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch4PercentOfTotalCellArea = CoEx(f,1).Total.Ch1Ch4AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
            CoEx(f,1).Total.Ch1Ch4PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch1Ch4AreaSum/sum(Image.BW1,'all')*100;
            
            CoEx(f,1).Total.Ch2Ch4AreaSum = sum(CoExImages.Ch2Ch4Total,'all');
            CoEx(f,1).Total.Ch2Ch4PercentOfTotalCh2Area = CoEx(f,1).Total.Ch2Ch4AreaSum/sum(ErosionProps(f,1).Ch2CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch4PercentOfTotalCh4Area = CoEx(f,1).Total.Ch2Ch4AreaSum/sum(ErosionProps(f,1).Ch4CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch4PercentOfTotalCellArea = CoEx(f,1).Total.Ch2Ch4AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
            CoEx(f,1).Total.Ch2Ch4PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch2Ch4AreaSum/sum(Image.BW1,'all')*100;
            
            CoEx(f,1).Total.Ch3Ch4AreaSum = sum(CoExImages.Ch3Ch4Total,'all');
            CoEx(f,1).Total.Ch3Ch4PercentOfTotalCh3Area = CoEx(f,1).Total.Ch3Ch4AreaSum/sum(ErosionProps(f,1).Ch3CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch3Ch4PercentOfTotalCh4Area = CoEx(f,1).Total.Ch3Ch4AreaSum/sum(ErosionProps(f,1).Ch4CellAreaSum(:,1),'all')*100;
            CoEx(f,1).Total.Ch3Ch4PercentOfTotalCellArea = CoEx(f,1).Total.Ch3Ch4AreaSum/sum(ErosionProps(f,1).TotalCellArea(:,1),'all')*100;
            CoEx(f,1).Total.Ch3Ch4PercentOfTotalMicrogelArea = CoEx(f,1).Total.Ch3Ch4AreaSum/sum(Image.BW1,'all')*100;
        else end
   
    for m = 1:Erosion.ErosionIterations
    %Creates co-expression bands for each band%
        CoExImages.Ch1Ch2Bands(:,:,m) = and(ErosionImages(f,1).Ch1CellMask(:,:,m),ErosionImages(f,1).Ch2CellMask(:,:,m));
        if Channels>2
            CoExImages.Ch1Ch3Bands(:,:,m) = and(ErosionImages(f,1).Ch1CellMask(:,:,m),ErosionImages(f,1).Ch3CellMask(:,:,m));
            CoExImages.Ch2Ch3Bands(:,:,m) = and(ErosionImages(f,1).Ch2CellMask(:,:,m),ErosionImages(f,1).Ch3CellMask(:,:,m));
        else end
        if Channels>3
            CoExImages.Ch1Ch4Bands(:,:,m) = and(ErosionImages(f,1).Ch1CellMask(:,:,m),ErosionImages(f,1).Ch4CellMask(:,:,m));
            CoExImages.Ch2Ch4Bands(:,:,m) = and(ErosionImages(f,1).Ch2CellMask(:,:,m),ErosionImages(f,1).Ch4CellMask(:,:,m));
            CoExImages.Ch3Ch4Bands(:,:,m) = and(ErosionImages(f,1).Ch3CellMask(:,:,m),ErosionImages(f,1).Ch4CellMask(:,:,m));
        else end

    %Performs co-expression analysis for each band%
        CoEx(f,1).Bands.Ch1Ch2AreaSum(m,1) = sum(CoExImages.Ch1Ch2Bands(:,:,m),'all');
        CoEx(f,1).Bands.Ch1Ch2PercentOfTotalCh1Area(m,1) = CoEx(f,1).Bands.Ch1Ch2AreaSum(m,1)/ErosionProps(f,1).Ch1CellAreaSum(m,1)*100;
        CoEx(f,1).Bands.Ch1Ch2PercentOfTotalCh2Area(m,1) = CoEx(f,1).Bands.Ch1Ch2AreaSum(m,1)/ErosionProps(f,1).Ch2CellAreaSum(m,1)*100;
        CoEx(f,1).Bands.Ch1Ch2PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch1Ch2AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
        CoEx(f,1).Bands.Ch1Ch2PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch1Ch2AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;

        if Channels>2
            CoEx(f,1).Bands.Ch1Ch3AreaSum(m,1) = sum(CoExImages.Ch1Ch3Bands(:,:,m),'all');
            CoEx(f,1).Bands.Ch1Ch3PercentOfTotalCh1Area(m,1) = CoEx(f,1).Bands.Ch1Ch3AreaSum(m,1)/ErosionProps(f,1).Ch1CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch3PercentOfTotalCh3Area(m,1) = CoEx(f,1).Bands.Ch1Ch3AreaSum(m,1)/ErosionProps(f,1).Ch3CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch3PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch1Ch3AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch3PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch1Ch3AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;

            CoEx(f,1).Bands.Ch2Ch3AreaSum(m,1) = sum(CoExImages.Ch2Ch3Bands(:,:,m),'all');
            CoEx(f,1).Bands.Ch2Ch3PercentOfTotalCh2Area(m,1) = CoEx(f,1).Bands.Ch2Ch3AreaSum(m,1)/ErosionProps(f,1).Ch2CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch3PercentOfTotalCh3Area(m,1) = CoEx(f,1).Bands.Ch2Ch3AreaSum(m,1)/ErosionProps(f,1).Ch3CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch3PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch2Ch3AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch3PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch2Ch3AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;
        else
        end

        if Channels>3
            CoEx(f,1).Bands.Ch1Ch4AreaSum(m,1) = sum(CoExImages.Ch1Ch4Bands(:,:,m),'all');
            CoEx(f,1).Bands.Ch1Ch4PercentOfTotalCh1Area(m,1) = CoEx(f,1).Bands.Ch1Ch4AreaSum(m,1)/ErosionProps(f,1).Ch1CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch4PercentOfTotalCh4Area(m,1) = CoEx(f,1).Bands.Ch1Ch4AreaSum(m,1)/ErosionProps(f,1).Ch4CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch4PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch1Ch4AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
            CoEx(f,1).Bands.Ch1Ch4PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch1Ch4AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;

            CoEx(f,1).Bands.Ch2Ch4AreaSum(m,1) = sum(CoExImages.Ch2Ch4Bands(:,:,m),'all');
            CoEx(f,1).Bands.Ch2Ch4PercentOfTotalCh2Area(m,1) = CoEx(f,1).Bands.Ch2Ch4AreaSum(m,1)/ErosionProps(f,1).Ch2CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch4PercentOfTotalCh4Area(m,1) = CoEx(f,1).Bands.Ch2Ch4AreaSum(m,1)/ErosionProps(f,1).Ch4CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch4PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch2Ch4AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
            CoEx(f,1).Bands.Ch2Ch4PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch2Ch4AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;

            CoEx(f,1).Bands.Ch3Ch4AreaSum(m,1) = sum(CoExImages.Ch3Ch4Bands(:,:,m),'all');
            CoEx(f,1).Bands.Ch3Ch4PercentOfTotalCh3Area(m,1) = CoEx(f,1).Bands.Ch3Ch4AreaSum(m,1)/ErosionProps(f,1).Ch3CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch3Ch4PercentOfTotalCh4Area(m,1) = CoEx(f,1).Bands.Ch3Ch4AreaSum(m,1)/ErosionProps(f,1).Ch4CellAreaSum(m,1)*100;
            CoEx(f,1).Bands.Ch3Ch4PercentOfTotalCellArea(m,1) = CoEx(f,1).Bands.Ch3Ch4AreaSum(m,1)/ErosionProps(f,1).TotalCellArea(m,1)*100;
            CoEx(f,1).Bands.Ch3Ch4PercentOfTotalBandArea(m,1) = CoEx(f,1).Bands.Ch3Ch4AreaSum(m,1)/ErosionProps(f,1).BandArea(m,1)*100;
        else
        end
    end %End of co-expression analysis by band %%
    else
    end %End of all co-expression analyses %%      

    %% Channel Mask Image Saving %%
    ...
    close all;    
end %%End Analysis Loop for Image.
 ...
 save('CoExpressionAnalysisResults.mat','CoEx','-v7.3');
